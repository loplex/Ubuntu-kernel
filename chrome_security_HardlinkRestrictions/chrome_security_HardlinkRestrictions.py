...
class chrome_security_HardlinkRestrictions(test.test):
    ...
    def run_once(self):
        ...
        rc = utils.system("su -c 'ln /etc/shadow /tmp/evil-hardlink' phablet",
                          ignore_status=True)
        if rc != 1 or os.path.exists("/tmp/evil-hardlink"):
            self._failed("phablet user was able to create malicious hardlink")

        # Test hardlink restrictions.
        self._check_hardlinks(user='phablet')
